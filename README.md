...
`phys_scene_all.json` is the unsplit full annotation file. It should not contain a fixed `split` field, so the train/test partition can be regenerated later with a different seed or ratio.
...
Important: predicate-frequency reweighting uses only `phys_scene_train.json`. It must not use `phys_scene_all.json` or `phys_scene_test.json`, because that would leak test-set distribution information.
