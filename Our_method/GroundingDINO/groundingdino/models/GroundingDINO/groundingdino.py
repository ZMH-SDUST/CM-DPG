...
from .relation_calibration import load_predicate_counts
...
class GroundingDINO(nn.Module):
    ...
    def __init__(
        ...
        self.use_relation_adaptive_calibration = use_relation_adaptive_calibration
        self.relation_adaptive_delta = relation_adaptive_delta
        self.relation_adaptive_eps = relation_adaptive_eps
        self.relation_visual_weight = relation_visual_weight
        self.relation_spatial_weight = relation_spatial_weight
        self.register_buffer(
            "predicate_counts",
            torch.as_tensor(predicate_counts if predicate_counts is not None else [], dtype=torch.float32),
            persistent=False,
        )
        ...
        self.spatial_head = nn.Sequential(
         ...
         nn.Linear(128, num_rln_cat), nn.ReLU(),
        )

        self.spatial_head_ovr = nn.Sequential(
            nn.Linear(36, 128), nn.ReLU(),
    ...
    def forward(self, samples: NestedTensor, targets: List = None, **kw):
        ...
        ''' Step 1: get captions from labels or predefined captions. '''
        if targets is None:
            captions = kw["captions"]
            rel_captions = kw['rel_captions'] if 'rel_captions' in kw else []
        else:
            # Entered in test mode.
            # captions is the ordered list of all object names.
            captions = [t["caption"] for t in targets]
            # rel_captions is the ordered list of all relation names.
            rel_captions = [t['rel_caption'] for t in targets] if 'rel_caption' \
                                                                  in targets[0] else []

        ''' Step 2: encode captions from labels (the object category set). '''
        # encoded_text: [1,309,256]
        ...
        rel_text_dict = None
        ''' Step 3: encode relation_captions only in the zero-shot setting. '''
        if self.do_sgg and self.sgg_mode != 'full':
            # encoded_text: [1,124,256]
            rel_text_dict = self.encode_captions(rel_captions, samples.device, encode_relation=True)
        # The number of object/relation encodings is not equal to the number of categories.

        # In the zero-shot setting, append object/relation encodings and metadata to the original object dictionary.
        concat_rel_text = True
        if rel_text_dict is not None and concat_rel_text:
            ...
            # Concatenate masks from the two dictionary types.
        ...
        ''' Step 4: perform visual encoding for the image. '''
        if isinstance(samples, (list, torch.Tensor)):
            samples = nested_tensor_from_tensor_list(samples)

        # backbone：Swin-transformer
        # Extract features and positions from three levels; each level has shape [b, c, h, w].
        features, poss = self.backbone(samples)

        # Map channel dimensions from different feature levels to 256.
        srcs = []
        masks = []
        ...
        # Entered during training and skipped during testing.
        use_dn = self.dn_number > 0 and targets is not None
        if use_dn:  # True
            # One key difference between DINO and DN-DETR is that DINO handles pattern embeddings in the detector forward pass and uses learnable target embeddings, so this function is modified accordingly.
            input_query_label, input_query_bbox, attn_mask, dn_meta = \
                prepare_for_cdn(dn_args=(targets, self.dn_number, self.dn_label_noise_ratio, self.dn_box_noise_scale),
                                training=self.training, num_queries=self.num_queries, num_classes=self.num_classes,
                                hidden_dim=self.hidden_dim, label_enc=self.label_enc)
            # input_query_label [4,198,256]
            # input_query_bbox [4,198,4]
            # attn_mask [1098, 1098]
            # dn_meta: pad_size 198,num_dp_group 11
        else:
            # assert targets is None
            input_query_bbox = input_query_label = attn_mask = dn_meta = None

            # Input backbone outputs, queries, attention masks, and semantic caption embeddings.
        ...
        # Object features output by the decoder [6, 4, 1098, 256].
        # Relation features output by the decoder [6, 4, 1, 256].
        # Object box information output by the decoder [7, 4, 1098, 4].
        # Top-k object features output by the encoder [1, 4, 900, 256].
        # Top-k object box information output by the encoder [1, 4, 900, 4].
        # Top-k object proposals output by the encoder [4, 900, 4].

        # split via sep_len
        # This was merged earlier and split again here; it may be redundant.
        if rel_text_dict is not None and concat_rel_text:
            rel_text_dict = {}
            for k in text_dict.keys():
            ...
            rel_text_dict['text_self_attention_masks'] = text_dict['text_self_attention_masks'][:, sep_len[0]:,
                                                         sep_len[0]:]
        ...
        ):  # Predicted object boxes, new box layer, and predicted object features.

            # Convert decoder object features into new box information.
            layer_delta_unsig = layer_bbox_embed(layer_hs)
            # Use new box information to refine the previous predicted boxes.
            layer_outputs_unsig = layer_delta_unsig + inverse_sigmoid(layer_ref_sig)
            layer_outputs_unsig = layer_outputs_unsig.sigmoid()  # [1,900,4]
            outputs_coord_list.append(layer_outputs_unsig)
        outputs_coord_list = torch.stack(outputs_coord_list)  # [6, 1, 900, 4]

        # Convert decoder object features into object category information.
        # Use similarity to text embeddings instead of softmax.
        outputs_class = torch.stack(
            [
                layer_cls_embed(layer_hs, text_dict)
                for layer_cls_embed, layer_hs in zip(self.class_embed, hs)
            ]
        )  # [6, 1, 900, 512]; the first 309 values are valid and the remaining 512-309 values are negative infinity.

        # Not entered during testing.
        if self.dn_number > 0 and dn_meta is not None:
            # Take predicted features after pad_size; entries before it belong to known classes, while later entries are predictions.
            hs_obj = [e[:, dn_meta['pad_size']:, :] for e in hs]
            # Select predicted classes and boxes after pad_size.
            outputs_class, outputs_coord_list = dn_post_process( \
                outputs_class, outputs_coord_list,
                dn_meta, self.aux_loss, self._set_aux_loss)
            # [6,4,900,512], [6,4,900,4]
        else:
            hs_obj = hs

        # Keep only the final decoder-layer results.
        # [1, 900, 512], [1, 900, 4]
        out = {"pred_logits": outputs_class[-1], "pred_boxes": outputs_coord_list[-1]}

        # for intermediate outputs
        if self.aux_loss:
            # Five levels, each containing outputs similar to the above.
            out['aux_outputs'] = self._set_aux_loss(outputs_class, outputs_coord_list)

        # Convert encoder outputs to category information as well.
        if hs_enc is not None:
            # Box information output by the encoder [1, 900, 4].
            interm_coord = ref_enc[-1]
            # Encoder output features are also converted to category information through semantic similarity [1, 900, 512].
        ...
        out['input_ids'] = text_dict['input_ids']  # tokenized['input_ids'] [4,309]
        if self.do_sgg:
            # Relation features from the last decoder layer.
            out['hs_rln'] = hs_rln[-1]
            # Object features from the last decoder layer.
            out['hs_obj'] = hs_obj[-1]
            # Semantic encodings for relation labels.
...
class PostProcess(nn.Module):
    ...
    def __init__(self, num_select=100, nms_iou_threshold=-1,
        ...
        self.predicate_counts = kwargs.get("predicate_counts", None)
        ...
        self.use_text_labels = kwargs.get("use_text_labels", False)  # True
    ...
    def forward(self, outputs, target_sizes, not_to_xyxy=False, test=False, cat_list=None, gt_dicts=None):
        ...
        if not_to_xyxy:
            ...
            boxes = box_ops.box_cxcywh_to_xyxy(boxes) # [1,300,4]
        ...
        if self.nms_iou_threshold > 0:
            item_indices = [batched_nms(b, s, l, iou_threshold=self.nms_iou_threshold) for b, s, l in
            ...
            if self.do_sgg:
                results = [{'scores': s[:self.detections_per_img],
                            ...
                            'obj_token': ot[:self.detections_per_img]} for s, l, b, ot in
                           zip(scores, labels, boxes, obj_token)]
            else:
                results = [{'scores': s[:self.detections_per_img],
                            'labels': l[:self.detections_per_img],
        ...
        if self.do_sgg:
            ...
            sgg_out = graph_infer(results, self.rln_proj, self.rln_classifier, self.spatial_head, self.spatial_head_ovr,
                                  self.rln_freq_bias,
                                  ...
                                  use_relation_adaptive_calibration=self.use_relation_adaptive_calibration,
                                  predicate_counts=self.predicate_counts,
                                  relation_adaptive_delta=self.relation_adaptive_delta,
                                  relation_adaptive_eps=self.relation_adaptive_eps,
                                  relation_visual_weight=self.relation_visual_weight,
                                  relation_spatial_weight=self.relation_spatial_weight,
                                  save_features=False
                                  )
...
def build_groundingdino(args):
    # Use swin_T_224_1k as the backbone; frozen by default.
    backbone = build_backbone(args)
    frozen_backbone = getattr(args, "frozen_backbone", False)  # Freeze the backbone.
    if frozen_backbone:
        print("*" * 10, " Freeze backbone !", "*" * 10)
        for name, p in backbone.named_parameters():
            p.requires_grad = False

    # Model backbone with an encoder-decoder architecture; the encoder includes visual, text, and fusion components.
    transformer = build_transformer(args)  # Deformable Transformer

    # Some parameter settings.
    dn_labelbook_size = args.dn_labelbook_size  # 2000
    dec_pred_bbox_embed_share = args.dec_pred_bbox_embed_share  # True
    sub_sentence_present = args.sub_sentence_present  # True

    # SGG settings for the transition from object detection to SGG.
    do_sgg = getattr(args, "do_sgg", False)  # True
    rln_pretraining = getattr(args, "rln_pretraining", False)  # False
    sgg_mode = getattr(args, "sgg_mode", "full")  # full
    num_rln_cat = getattr(args, "num_rln_cat", 51)  # 51
    rln_freq_bias = getattr(args, "rln_freq_bias", None)  # Contents from vg_stats.pt.
    use_relation_adaptive_calibration = getattr(args, "use_relation_adaptive_calibration", False)
    relation_adaptive_delta = getattr(args, "relation_adaptive_delta", 1.0)
    relation_adaptive_eps = getattr(args, "relation_adaptive_eps", 1e-6)
    relation_visual_weight = getattr(args, "relation_visual_weight", 0.5)
    relation_spatial_weight = getattr(args, "relation_spatial_weight", 0.5)
    predicate_counts = load_predicate_counts(args, num_rln_cat) if use_relation_adaptive_calibration else None
    if rln_freq_bias is not None:
        rln_freq_bias = torch.load(rln_freq_bias)

    # Define the full open-set object detection model by composing the components above.
    model = GroundingDINO(
        backbone,
        transformer,
        ...
        use_relation_adaptive_calibration=use_relation_adaptive_calibration,
        predicate_counts=predicate_counts,
        relation_adaptive_delta=relation_adaptive_delta,
        relation_adaptive_eps=relation_adaptive_eps,
        relation_visual_weight=relation_visual_weight,
        relation_spatial_weight=relation_spatial_weight,
    )

    # matcher: HungarianMatcher, matching predictions with ground truth.
    matcher = build_matcher(args, tokenizer=model.tokenizer)  # Bert tokenizer fast
    ...
    # Denoising training weights; this strategy is used by default.
    if args.use_dn:
    ...
    # TODO this is a hack
    # Use auxiliary losses by default.
    if args.aux_loss:
        aux_weight_dict = {}
    ...
    if args.two_stage_type != 'no':  # standard
        interm_weight_dict = {}
        try:
    ...
    # Loss types: labels, boxes, and edges.
    losses = ['labels', 'boxes']
    if use_masks:
        losses += ["masks"]
    if do_sgg:
    ...
    # The following branch is not entered.
    ...
    # Define the loss function, including result matching and loss computation.
    ...
    # Convert model outputs to the format required by COCO.
    use_text_labels = getattr(args, "use_text_labels", True)  # True
    ...
    # Freeze BERT components in the model.
    for name, p in model.named_parameters():
        if 'bert' in name:
            p.requires_grad = False
    ...
    # Only feature maps and the newly added components below (which extend the detector to relation detection) are trainable.
