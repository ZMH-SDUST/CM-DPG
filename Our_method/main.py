...
def get_args_parser():
    ...
    #dis
    # parser.add_argument('--use_distill', default=True, type=bool, help='use teacher model or not')
    # parser.add_argument('--unsupervised_distill', default=True, type=bool, help='')
    return parser
...
def main(gpu, ngpus_per_node, args):
    ...
    # Define the output directory and logger.
    ...
    # Save system arguments to the log file for record keeping.
    if args.rank == 0:
    ...
    # Adjust and print system arguments.
    ...
    # Load the pretrained DETR model (disabled by default).
    if args.frozen_weights is not None:
        assert args.masks, "Frozen training is meant for segmentation only"
        checkpoint = torch.load(args.frozen_weights, map_location='cpu')
        model_without_ddp.detr.load_state_dict(checkpoint['model'])

    # Print the relation-layer structure in the model.
    rln_proj = getattr(model, "rln_proj", None)
    rln_classifier = getattr(model, "rln_classifier", None)
    rln_freq_bias = getattr(model, "rln_freq_bias", None)  # [22801,51]
    ...
    # load pre-trained weights of DINO
    if (not args.resume) and args.pretrain_model_path:
        ...
        pre_w = _tmp_st['bert.embeddings.word_embeddings.weight']  # [30522, 768]
        ...
        # The pretrained weights miss transformer.decoder.layers relation_cross_attn parameters (in/out proj weight/bias) and norm12 parameters (weight/bias).
        # transformer.decoder.rln_embed.weight, rln_embed.weight,'rln_proj.layers.0/1.weight/bias
        # rln_classifier.weight, rln_classifier.bias, rln_freq_bias.obj_baseline.weight
        ...
        rln_text_proj = getattr(model_without_ddp, "rln_text_proj", None)  # None
    ...
    # Load weights by default; this can be set to False.
    # Temporarily disable weight loading.
    # args.resume = False
    if args.resume:
        if args.resume.startswith('https'):
            ...
            # Automatically load weights from the log directory.
        ...
        # Load optimizer state to support resume training.
    ...
    eval_before_train = getattr(args, "eval_before_train", True)  # False
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # Save the corresponding weights and settings after each epoch.
        ...
        # Evaluate after each epoch; print metrics without saving them.
        test_stats, coco_evaluator = evaluate(
            model, criterion, postprocessors, data_loader_val, base_ds, device, args.output_dir,
            wo_class_error=wo_class_error, args=args, logger=(logger if args.save_log else None)
        )
        ...
        # Evaluate the best model using the R@50 metric.
        if _isbest:
        ...
        # Save the log information for this epoch.
        if args.output_dir and utils.is_main_process():
            with (output_dir / "log.txt").open("a") as f:
                f.write(json.dumps(log_stats) + "\n")
    ...
    # Release copied intermediate files.
