...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    teacher_update_interval = getattr(args, "teacher_update_interval", 1000)  # 1000
    momentum_t = getattr(args, "teacher_momentum", 0.999)
    teacher_update = getattr(args, "teacher_update", False)  # False
    ...
    use_text_labels = getattr(args, "use_text_labels", False)  # True
    if use_text_labels:
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header, logger=logger):
        ...
        with torch.cuda.amp.autocast(enabled=args.amp):
            if need_tgt_for_training:
                # predict_logits: [4, 900, 512]
                # predict_boxes: [4, 900, 4]
                # hs_obj: [4, 900, 256]
                # hs_rln: [4, 1, 256]
                # aux_outputs: 5 * [above 4 items]
                # input_ids: [4, 309]
                outputs = model(samples, targets, global_iter=args.global_iter)  # Coming in
            else:
                outputs = model(samples, global_iter=args.global_iter)

            if model_t is not None:  # None
                with torch.no_grad():
                    if need_tgt_for_training:
...
def evaluate(model, criterion, postprocessors, data_loader, base_ds, device, output_dir, wo_class_error=False,
    ...
    metric_logger = utils.MetricLogger(delimiter="  ")
    if not wo_class_error:  # coming in
        metric_logger.add_meter('class_error', utils.SmoothedValue(window_size=1, fmt='{value:.2f}'))
    header = 'Test:'
    ...
    try:
        ...
        coco_evaluator = None # None

    sgg_evaluator = None
    if args.dataset_file in ["vg", "phys_scene", "oicap", "coco", "coco_flickr30k", "coco_flickr30k_sbucaptions"]:
        ...
        iou_types = tuple(set(iou_types))  # relation,bbox
        num_rel_category = getattr(args, "num_rel_category", getattr(args, "num_rln_cat", 51))

        # Set the evaluation mode here.
        # iou_types: bbox, relation
        # mode: predcls, sgcls, sgdet
        # Set this to True or False for OVD/OVR.
    ...
    for samples, targets in metric_logger.log_every(data_loader, 10, header, logger=logger):
        ...
        # Store weighted and unweighted losses.
        ...
        if postprocessors['bbox'].use_gt_box:  # False
            ...
            results = postprocessors['bbox'](outputs, orig_target_sizes)
        # [scores: [100], labels: [100], boxes: [100, 4]] x B; relations are enumerated as 100*100 - 100.
        ...
        # Associate image IDs with prediction results.
        ...
        # This branch is not entered.
        if coco_evaluator is not None: # None
            coco_evaluator.update(res)

        if sgg_evaluator is not None:
            # Compute and store prediction metrics.
    ...
    # Synchronize and average loss results.
    metric_logger.synchronize_between_processes()
    print("Averaged stats:", metric_logger)
    # Skip these two if branches.
    ...
    # Print metrics and reset the evaluator.
    sgg_res = None
    if sgg_evaluator is not None:
        # Print metrics.
        sgg_res = sgg_evaluator.accumulate()  # Print SGG metrics and return the R@50 result.
        sgg_evaluator.summarize()  # Print overall detection metrics (AP, AR, and box metrics), plus AP and AP50 for each class.
        sgg_evaluator.reset()

    # Skip the following branch.
    panoptic_res = None
    if panoptic_evaluator is not None:  # None
        panoptic_res = panoptic_evaluator.summarize()
    # loss + coco_eval_bbox + R@50
    ...
    if panoptic_res is not None:  # None
