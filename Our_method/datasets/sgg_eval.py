...
class SggEvaluator(object):
    def __init__(self, dataset, iou_types=("bbox", "relation"), 
        ...
        self.num_rel_category = num_rel_category # 51
        self.iou_types = iou_types  # box, relation
        ...
        self.useCats = useCats # True
        self.multiple_preds = multiple_preds # False
        # Zero-shot triplet list in index form.
        zeroshot_triplet_path = os.path.join('./data/visual_genome/', "zeroshot_triplet.pytorch")
        if os.path.exists(zeroshot_triplet_path):
            self.zeroshot_triplet = torch.load(
                zeroshot_triplet_path,
    ...
    def update_coco(self, predictions):
        ...
        # Post-processing for 100 predicted objects.
        ...
        with self.lock_coco:
            ...
            # Evaluate the given image and store the result in eval_imgs.
    ...
    def get_sgg_evaluator(self, mode):
        ...
        if self.ovd_enabled:
            # ovd. zero shot
            eval_ovd_zeroshot_recall = OvdSGZeroShotRecall(
                result_dict,
                unseen_obj_cats=list(getattr(self.dataset, "novel_object_ids", [])) or None,
            )
        ...
        if self.ovr_enabled:
            # ovr. zero shot
            eval_ovr_zeroshot_recall = OvrSGZeroShotRecall(
                result_dict,
                unseen_rels=list(getattr(self.dataset, "novel_predicate_ids", [])) or None,
            )
...
def evaluate_relation_of_one_image(groundtruth, prediction, global_container, evaluator):
    ...
    local_container = {}
    # Store box, class, and predicate information from predictions and ground truth.
