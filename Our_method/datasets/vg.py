...
VG150_BASE_OBJ_CATEGORIES = ['__background__', 'tile', 'drawer', 'men', 'railing', 'stand', 'towel', 'sneaker',
                             'vegetable', 'screen', 'vehicle', 'animal', 'kite', 'cabinet', 'sink', 'wire', 'fruit',
                             'curtain', 'lamp', 'flag', 'pot', 'sock', 'boot', 'guy', 'kid', 'finger', 'basket', 'wave',
                             'lady', 'orange', 'number', 'toilet', 'post', 'room', 'paper', 'mountain', 'paw', 'banana',
                             'rock', 'cup', 'hill', 'house', 'airplane', 'plant', 'skier', 'fork', 'box', 'seat',
                             'engine', 'mouth', 'letter', 'windshield', 'desk', 'board', 'counter', 'branch', 'coat',
                             'logo', 'book', 'roof', 'tie', 'tower', 'glove', 'sheep', 'neck', 'shelf', 'bottle', 'cap',
                             'vase', 'racket', 'ski', 'phone', 'handle', 'boat', 'tire', 'flower', 'child', 'bowl',
                             'pillow', 'player', 'trunk', 'bag', 'wing', 'light', 'laptop', 'pizza', 'cow', 'truck',
                             'jean', 'eye', 'arm', 'leaf', 'bird', 'surfboard', 'umbrella', 'food', 'people', 'nose',
                             'beach', 'sidewalk', 'helmet', 'face', 'skateboard', 'motorcycle', 'clock', 'bear']

VG150_NOVEL2BASE = {'bed': [], 'bench': ['seat'], 'bike': ['vehicle', 'motorcycle'], 'boy': ['men'], 'building': [],
                    'bus': ['vehicle'], 'car': ['vehicle'], 'cat': ['animal'], 'chair': ['seat'], 'dog': ['animal'],
                    'door': [], 'ear': [], 'elephant': ['animal'], 'fence': [], 'giraffe': ['animal'], 'girl': ['lady'],
                    'glass': [], 'hair': [], 'hand': ['paw'], 'hat': ['cap'], 'head': ['men'], 'horse': ['animal'],
                    'jacket': ['coat'], 'leg': ['branch'], 'man': ['animal'], 'pant': [], 'person': ['people'],
                    'plane': ['airplane'], 'plate': ['food'], 'pole': [], 'shirt': [], 'shoe': [], 'short': [],
                    'sign': ['house'], 'snow': [], 'street': ['sidewalk'], 'table': ['board', 'desk'], 'tail': [],
                    'track': ['railing'], 'train': ['vehicle'], 'tree': ['plant'], 'wheel': ['tire'], 'window': [],
                    'woman': ['lady'], 'zebra': ['animal']}

PREDICATE_BG = '[UNK]'  # speical token

VG150_PREDICATES = [PREDICATE_BG, "above", "across", "against", "along", "and", "at", "attached to", "behind",
...
VG150_BASE_PREDICATE = [PREDICATE_BG, "between", "to", "made of", "looking at", "along", "laying on", "using",
                        "carrying", "against", "mounted on", "sitting on", "flying in", "covering", "from", "over",
                        "near", "hanging from", "across", "at", "above", "watching", "covered in", "wearing", "holding",
                        "and", "standing on", "lying on", "growing on", "under", "on back of", "with", "has",
                        "in front of", "behind", "parked on"]

VG150_NOVEL_PREDICATE = ["belonging to", "part of", "riding", "walking in", "in", "of", "painted on", "playing", "for",
                         "walking on", "says", "attached to", "eating", "on", "wears"]
...
class VGDataset(torch.utils.data.Dataset):
    def __init__(self, split, img_dir, roidb_file, dict_file, image_file, transforms=None,
        ...
        self.dict_file = dict_file  # VG-SGG-dicts.json
        self.roidb_file = roidb_file  # VG-SGG.h5
        self.roidb_key = roidb_key  # split_GLIPunseen
        self.image_file = image_file  # image_data.json
        self.filter_non_overlap = filter_non_overlap and self.split == 'train'  # True for train
        self.filter_duplicate_rels = filter_duplicate_rels and self.split == 'train'  # True for train
        self.transforms = transforms
        self.ovd_mode = ovd_mode
        self.ovr_mode = ovr_mode
        self.use_distill = use_distill # False
        self.unsupervised_distill = unsupervised_distill

        '''*****************************************************************************************'''
        # Step 1: load object and predicate index/name information.
        ...
        '''*****************************************************************************************'''
        # Step 2: read annotation information.
        ...
        # Image names and basic attributes.
        ...
        # GPT4SGG
        self.use_gpt4sgg = use_gpt4sgg and self.split == 'train' # False
        ...
        self.annotations = [] # Store each image annotation in indexed form.
        ...
        self.use_text_labels = use_text_labels # True
    ...
    def __getitem__(self, index):
        ...
        if self.use_text_labels:
            ...
            # Use the base object set only for the OVD training split.
            ...
            # Use the base predicate set only for the OVR training split.
            ...
            if self.use_distill and (not self.unsupervised_distill):
                all_rels = VG150_PREDICATES[
                           1:]  # we release the constraint of OvR mode, to let the model access to all categories but does not know which one is base or novel. And novel categories are removed in annotations.
    ...
    for i in range(len(image_index)):
        ...
        for r in rels:
            ...
            if r[2] not in keep_rels:
                keep_rels[r[2]] = 0
            keep_rels[r[2]] += 1
    # [108073], [56224, N, 4], [56224, N, 4], [], [56224, M, 3], object count dictionary [120], predicate count dictionary [50].
