...
class OvdSGZeroShotRecall(SceneGraphEvaluation):
    def __init__(self, result_dict, **kwargs):
        ...
        self.novel_count = 0
        self.all_count = 0
        self.unseen_obj_cats = kwargs.get("unseen_obj_cats", None)
    ...
    def prepare_zeroshot(self, global_container, local_container):
        ...
        unseen_obj_cats = self.unseen_obj_cats or [9, 10, 11, 20, 22, 23, 26, 27, 28, 37, 38, 40, 41, 45, 52, 53, 54, 57, 58, 60, 61, 64, 66, 74, 78, 87, 91, 95, 97, 99, 111, 112, 113, 115, 121, 124, 126, 127, 134, 135, 136, 144, 145, 149, 150]
        ...
        for k in self.result_dict[mode + self.key]:
            ...
            if len(self.zeroshot_idx) > 0:
                if not isinstance(match, (list, tuple)):
                    ...
                    match_list = match
                # Intersection items that are both matched and included in the zero-shot list.
                zeroshot_match = len(self.zeroshot_idx) + len(match_list) - len(set(self.zeroshot_idx + match_list))
                zero_rec_i = float(zeroshot_match) / float(len(self.zeroshot_idx))
...
class OvrSGZeroShotRecall(OvdSGZeroShotRecall):
    def __init__(self, result_dict, **kwargs):
        super().__init__(result_dict, **kwargs)
        self.novel_count = 0
        self.all_count = 0
        self.unseen_rels = kwargs.get("unseen_rels", None) or \
