...
def build_model_main(args):
    ...
    assert args.modelname in MODULE_BUILD_FUNCS._module_dict, "ERROR: modelname:{} not in models:{}".format(
        args.modelname, MODULE_BUILD_FUNCS._module_dict)
...
def main(gpu, ngpus_per_node, args):
    ...
    pair_visual_attn = getattr(model, "pair_visual_attn", None)
    pair_visual_norm = getattr(model, "pair_visual_norm", None)
    pair_dif_proj = getattr(model, "pair_dif_proj", None)
    pair_sum_proj = getattr(model, "pair_sum_proj", None)
    pair_arg_proj = getattr(model, "pair_arg_proj", None)
    spatial_head = getattr(model, "spatial_head", None)
    spatial_head_ovr = getattr(model, "spatial_head_ovr", None)
    ...
    if getattr(args, "use_distill", False):
        ...
        pair_dif_proj_teacher = getattr(model_t, "pair_dif_proj", None)
        pair_sum_proj_teacher = getattr(model_t, "pair_sum_proj", None)
        pair_arg_proj_teacher = getattr(model_t, "pair_arg_proj", None)
    else:
        model_t = None
        rln_proj_teacher = None
        pair_dif_proj_teacher = None
        pair_sum_proj_teacher = None
        pair_arg_proj_teacher = None
    ...
    if args.distributed:
        ...
        if rln_freq_bias is not None:
            rln_freq_bias = DDP(rln_freq_bias, device_ids=[args.gpu],
                                find_unused_parameters=args.find_unused_params)
        if pair_visual_attn is not None:
            pair_visual_attn = DDP(pair_visual_attn, device_ids=[args.gpu],
                                   find_unused_parameters=args.find_unused_params)

        if pair_visual_norm is not None:
            pair_visual_norm = DDP(pair_visual_norm, device_ids=[args.gpu],
                                   find_unused_parameters=args.find_unused_params)
        if pair_dif_proj is not None:
            pair_dif_proj = DDP(pair_dif_proj, device_ids=[args.gpu],
                                find_unused_parameters=args.find_unused_params)

        if pair_sum_proj is not None:
            pair_sum_proj = DDP(pair_sum_proj, device_ids=[args.gpu],
                                find_unused_parameters=args.find_unused_params)

        if pair_arg_proj is not None:
            pair_arg_proj = DDP(pair_arg_proj, device_ids=[args.gpu],
                                find_unused_parameters=args.find_unused_params)
        ...
        if spatial_head_ovr is not None:
            spatial_head_ovr = DDP(
                spatial_head_ovr,
                device_ids=[args.gpu],
                find_unused_parameters=args.find_unused_params,
            )
    ...
    criterion.rln_classifier = rln_classifier
    criterion.rln_freq_bias = rln_freq_bias
    criterion.pair_visual_attn = pair_visual_attn
    criterion.pair_visual_norm = pair_visual_norm
    criterion.pair_dif_proj = pair_dif_proj
    criterion.pair_sum_proj = pair_sum_proj
    criterion.pair_arg_proj = pair_arg_proj
    criterion.pair_dif_proj_teacher = pair_dif_proj_teacher
    criterion.pair_sum_proj_teacher = pair_sum_proj_teacher
    criterion.pair_arg_proj_teacher = pair_arg_proj_teacher
    criterion.spatial_head = spatial_head
    criterion.spatial_head_ovr = spatial_head_ovr
    allow_self_relations = not getattr(args, "phys_scene_exclude_self_relations", True)
    criterion.allow_self_relations = allow_self_relations
    postprocessors['bbox'].rln_proj = rln_proj
    postprocessors['bbox'].rln_classifier = rln_classifier
    postprocessors['bbox'].rln_freq_bias = rln_freq_bias
    postprocessors['bbox'].pair_visual_attn = pair_visual_attn
    postprocessors['bbox'].pair_visual_norm = pair_visual_norm
    postprocessors['bbox'].pair_dif_proj = pair_dif_proj
    postprocessors['bbox'].pair_sum_proj = pair_sum_proj
    postprocessors['bbox'].pair_arg_proj = pair_arg_proj
    postprocessors['bbox'].spatial_head = spatial_head
    postprocessors['bbox'].spatial_head_ovr = spatial_head_ovr
    postprocessors['bbox'].allow_self_relations = allow_self_relations
