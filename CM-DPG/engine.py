...
def _get_predicate_freq(dataset):
    if hasattr(dataset, "predicate_freq"):
        return dataset.predicate_freq

    if not hasattr(dataset, "ind_to_predicates"):
        return None

    freq = torch.ones(len(dataset.ind_to_predicates), dtype=torch.float32)

    if hasattr(dataset, "relationships"):
        for rels in dataset.relationships:
            if rels is None or len(rels) == 0:
                continue
            rels = torch.as_tensor(rels)
            if rels.ndim == 2 and rels.shape[1] >= 3:
                labels = rels[:, 2].long()
                labels = labels[(labels >= 0) & (labels < len(freq))]
                if len(labels) > 0:
                    freq += torch.bincount(labels, minlength=len(freq)).float()
        return freq

    if hasattr(dataset, "annotations"):
        for ann in dataset.annotations:
            edges = ann.get("edges", None)
...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    predicate_freq = _get_predicate_freq(data_loader.dataset)
    if predicate_freq is not None:
        criterion.predicate_freq = predicate_freq
...
def evaluate(model, criterion, postprocessors, data_loader, base_ds, device, output_dir, wo_class_error=False,
    ...
    sgg_evaluator = None
    if args.dataset_file in ["vg", "oicap", "coco", "coco_flickr30k", "coco_flickr30k_sbucaptions", "phys_scene",
                             "physscene"]:
