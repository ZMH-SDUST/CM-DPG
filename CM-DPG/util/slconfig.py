...
class SLConfig(object):
    ...
    def _file2dict(filename):
        ...
        if filename.lower().endswith('.py'):
            # Windows下固定的临时文件根目录
            fixed_temp_root = r'C:\Users\Zzou\Desktop\CM-DPG\Temp'
            os.makedirs(fixed_temp_root, exist_ok=True)

            # 在固定目录下创建一个独立的临时子目录
            with tempfile.TemporaryDirectory(
                    dir=fixed_temp_root) as temp_config_dir:
                ...
                temp_config_name = osp.basename(temp_config_path)
                ...
                try:
                    ...
                    # 清理导入路径及模块缓存
                    if temp_config_dir in sys.path:
        ...
        if BASE_KEY in cfg_dict:
            ...
            base_cfg_dict = SLConfig._merge_a_into_b(
                cfg_dict,
                base_cfg_dict
            )
            cfg_dict = base_cfg_dict
