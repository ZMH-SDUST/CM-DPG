...
class OvdSGZeroShotRecall(SceneGraphEvaluation):
    ...
    def prepare_zeroshot(self, global_container, local_container):
        ...
        unseen_obj_cats = global_container.get('unseen_obj_cats', None)
        if unseen_obj_cats is None:
            unseen_obj_cats = [9, 10, 11, 20, 22, 23, 26, 27, 28, 37, 38, 40, 41, 45, 52, 53, 54, 57, 58, 60, 61, 64,
                               66, 74, 78, 87, 91, 95, 97, 99, 111, 112, 113, 115, 121, 124, 126, 127, 134, 135, 136,
                               144, 145, 149, 150]
...
class OvrSGZeroShotRecall(OvdSGZeroShotRecall):
    ...
    def prepare_zeroshot(self, global_container, local_container):
        ...
        for i, (s, o, p) in enumerate(gt_triplets):
            ...
            if p in global_container.get('unseen_rels', self.unseen_rels):
                self.zeroshot_idx.append(i)
                self.novel_count += 1
            self.all_count += 1    
