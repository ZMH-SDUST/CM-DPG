...
class SggEvaluator(object):
    def __init__(self, dataset, iou_types=("bbox", "relation"), 
        ...
        if hasattr(self.dataset, "zeroshot_triplet"):
            self.zeroshot_triplet = np.asarray(self.dataset.zeroshot_triplet, dtype=np.int64)
        else:
            self.zeroshot_triplet = torch.load(os.path.join('./data/visual_genome/',
                                                            "zeroshot_triplet.pytorch"),
                                               map_location=torch.device("cpu")).long().numpy()
    ...
    def reset(self):
        ...
        try:
            ...
            self.global_container['unseen_obj_cats'] = getattr(self.dataset, "unseen_obj_cats", None)
            self.global_container['unseen_rels'] = getattr(self.dataset, "unseen_rels", None)
            self.do_sgg = True
        except:
            self.do_sgg = False 
