...
class GroundingDINO(nn.Module):
    ...
    def __init__(
        ...
        if self.do_sgg:
            ...
            self.pair_visual_attn = nn.MultiheadAttention(
                embed_dim=self.hidden_dim,
                num_heads=self.nheads,
                dropout=0.0,
                batch_first=False,
            )
            self.pair_visual_norm = nn.LayerNorm(self.hidden_dim)
            self.pair_dif_proj = MLP(self.hidden_dim, self.hidden_dim, self.hidden_dim, 2)
            self.pair_sum_proj = MLP(self.hidden_dim, self.hidden_dim, self.hidden_dim, 2)
            self.pair_arg_proj = MLP(self.hidden_dim * 2, 2048, self.hidden_dim, 2)

            extra_dim = 0  # if not self.rln_add_coords else self.hidden_dim
            input_dim = self.hidden_dim * (self.num_rln_queries + 1) + extra_dim
    ...
    def forward(self, samples: NestedTensor, targets: List = None, **kw):
        ...
        # )
        hs, hs_rln, reference, hs_enc, ref_enc, init_box_proposal, enc_memory, enc_mask = self.transformer(
            srcs, masks, input_query_bbox, poss,
            input_query_label, attn_mask, text_dict,
        )
...
class PostProcess(nn.Module):
    ...
    def __init__(self, num_select=100, nms_iou_threshold=-1,
        ...
        self.allow_self_relations = kwargs.get("allow_self_relations", False)
    ...
    def forward(self, outputs, target_sizes, not_to_xyxy=False, test=False, cat_list=None, gt_dicts=None):
        ...
        if self.do_sgg:
            ...
            _ = [result.update({
                'enc_memory': outputs['enc_memory'][batch_id],
                'enc_mask': outputs['enc_mask'][batch_id],
            })
                for batch_id, result in enumerate(results)]
            ...
            sgg_out = graph_infer(results, self.rln_proj, self.rln_classifier, self.spatial_head, self.spatial_head_ovr,
                                  ...
                                  self.pair_sum_proj,
                                  self.pair_arg_proj,
                                  self.pair_visual_attn,
                                  self.pair_visual_norm,
                                  self.rln_freq_bias,
                                  outputs['rel_text_dict'],
                                  self.name2predicates,
                                  self.tokenizer,
                                  ...
                                  use_classifier=self.rln_classifier is not None,
                                  save_features=False,
                                  allow_self_relations=self.allow_self_relations,
                                  )
